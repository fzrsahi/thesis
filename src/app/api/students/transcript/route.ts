...
} from "@/app/server/student/transcript/transcript.error";
import { uploadTranscript } from "@/app/server/student/transcript/upload.transcript.usecase";
...
export const POST = withAuth(
  async (request: NextRequest, session) => {
    try {
      ...
      await uploadTranscript(Number(session.user.id), result.data.transcript);
